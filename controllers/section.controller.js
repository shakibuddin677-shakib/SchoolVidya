...
export const createSection = async (req, res) => {
  try {
    ...
    // classId ek valid MongoDB ObjectId format mein hai ya nahi - yeh check pehle na karo to agla step (findById) crash kar sakta hai agar koi random string bhej de
    if (!mongoose.Types.ObjectId.isValid(classId)) {
      return res.status(400).json({ success: false, message: "Invalid classId" });
    }

    // "Foreign key" MongoDB khud check nahi karta (SQL ke uljat), isliye humein khud verify karna padta hai ki yeh Class sach mein exist karta hai
    ...
    // Agar classTeacherId diya hai, verify karo ki wo ek valid ObjectId hai aur sach mein ek "teacher" role wale User se belong karta hai
    if (classTeacherId) {
...
export const getAllSections = async (req, res) => {
  try {
    ...
    const sections = await Section.find(filter)
      // populate(field, "sirf yeh fields chahiye") - poora document nahi laate, sirf jitna zaroorat hai, taaki response halka rahe
